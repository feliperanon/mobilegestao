  ...
  return (
    <Container>
      ...
      <Box sx={{ mb: 4 }}>
        ...
        <TableContainer component={Paper}>
          <Table>
            ...
            <TableBody>
              {metrics.topClientsByTimePerVolume.map((client, index) => (
                <TableRow key={index}>
                  <TableCell>{client.cliente}</TableCell>
                  <TableCell>{client.avgTimePerVolume.toFixed(2)}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
      ...
      <Box sx={{ mb: 4 }}>
        ...
        <Bar
          data={{
            labels: metrics.topClientsByTotalTime.map(m => m.cliente),
            datasets: [{
              label: 'Tempo Total (min)',
              data: metrics.topClientsByTotalTime.map(m => m.time),
      ...
      <Box sx={{ mb: 4 }}>
        ...
        <Bar
          data={{
            labels: metrics.topClientsByVolume.map(m => m.cliente),
            datasets: [{
              label: 'Volume Total',
              data: metrics.topClientsByVolume.map(m => m.volume),
