    ...
    <Container>
      ...
      <Box component="form" onSubmit={handleSubmit} sx={{ mb: 4, display: 'flex', flexDirection: 'row', flexWrap: 'wrap', gap: 2 }}>
        ...
        <Autocomplete
          ...
          renderInput={(params) => <TextField {...params} label="Cliente" margin="normal" fullWidth required InputLabelProps={{ shrink: true }} />}
          sx={{ flex: 1 }}
        />
        <Autocomplete
          options={statusList}
          ...
          <TableBody>
            {deliveries.map((delivery) => (
              <TableRow key={delivery._id}>
                ...
                <TableCell>{delivery.cliente.join(', ')}</TableCell> {/* Tratamento como array */}
