...
const Dashboard = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post('http://localhost:5008/api/deliveries', {
        ...
        clientes: selectedClientes.map(cliente => cliente.nome),
        status: status?.nome,
        volume,
        tempoEstimado,
      });
  ...
  const handleDelete = async () => {
    try {
      ...
      const updatedDeliveries = deliveries.filter((delivery) => delivery._id !== deleteDeliveryId);
      setDeliveries(updatedDeliveries);
      closeDeleteDialog();
    } catch (err) {
      console.error(err);
    ...
    <Container>
      ...
      <Box component="form" onSubmit={handleSubmit} sx={{ mb: 4, display: 'flex', flexDirection: 'row', flexWrap: 'wrap', gap: 2 }}>
        ...
        <Autocomplete
          ...
          renderInput={(params) => <TextField {...params} label="Clientes" margin="normal" fullWidth required error={selectedClientes.length === 0} />}
          sx={{ flex: 1 }}
        />
        <Autocomplete
          options={statusList}
          ...
          <TableBody>
            {deliveries.map((delivery) => (
              <TableRow key={delivery._id}>
                ...
                <TableCell>{delivery.clientes.join(', ')}</TableCell>
      ...
      <Dialog open={deleteDialogOpen} onClose={closeDeleteDialog}>
        ...
        <DialogContent>
          <DialogContentText>Tem certeza que deseja excluir esta entrega?</DialogContentText>
        </DialogContent>
        <DialogActions>
