...
const Dashboard = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!entregador || selectedClientes.length === 0 || !status) {
    ...
    <Container>
      ...
      <Box component="form" onSubmit={handleSubmit} sx={{ mb: 4, display: 'flex', flexDirection: 'row', flexWrap: 'wrap', gap: 2 }}>
        ...
        <Autocomplete
          ...
          renderInput={(params) => <TextField {...params} label="Clientes" margin="normal" fullWidth required error={selectedClientes.length === 0} helperText={selectedClientes.length === 0 ? 'Selecione pelo menos um cliente' : ''} />}
          sx={{ flex: 1 }}
        />
        <Autocomplete
          options={statusList}
