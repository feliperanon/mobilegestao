    ...
    <Container>
      ...
      <Box component="form" onSubmit={handleSubmit} sx={{ mb: 4, display: 'flex', flexDirection: 'row', flexWrap: 'wrap', gap: 2 }}>
        <Autocomplete
          ...
          value={entregador}
          onChange={(event, newValue) => setEntregador(newValue ? newValue.nome : '')}
          renderInput={(params) => (
            <TextField {...params} label="Entregador" fullWidth required sx={{ width: 256, height: 56 }} />
          )}
        />
        <Autocomplete
          options={clientes}
          getOptionLabel={(option) => option.nome}
          value={cliente}
          onChange={(event, newValue) => setCliente(newValue ? newValue.nome : '')}
          renderInput={(params) => (
            <TextField {...params} label="Cliente" fullWidth required sx={{ width: 256, height: 56 }} />
          )}
        />
        <Autocomplete
          options={statusList}
          getOptionLabel={(option) => option.nome}
          value={status}
          onChange={(event, newValue) => setStatus(newValue ? newValue.nome : '')}
          renderInput={(params) => (
            <TextField {...params} label="Status" fullWidth required sx={{ flex: 1 }} />
          )}
        />
        <TextField
          label="Volume"
      ...
      <Dialog open={editDialogOpen} onClose={closeEditDialog}>
        ...
        <DialogContent>
          ...
          <Autocomplete
            ...
            onChange={(event, newValue) => setEditEntregador(newValue ? newValue.nome : '')}
            renderInput={(params) => (
              <TextField {...params} label="Entregador" fullWidth required margin="normal" />
            )}
          />
        </DialogContent>
        <DialogActions>
