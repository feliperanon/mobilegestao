...
const getDeliveries = async (req, res) => {
  try {
    const deliveries = await Delivery.find().sort({ iniciado: -1 });
...
const addDelivery = async (req, res) => {
  ...
  try {
    ...
    const tempoEmAndamento = 0; // Inicialmente, o tempo em andamento é zero.
...
const finalizeDelivery = async (req, res) => {
  try {
    const updatedDelivery = await Delivery.findByIdAndUpdate(req.params.id, { isActive: false, finalizado: new Date() }, { new: true });
...
module.exports = { getDeliveries, addDelivery, finalizeDelivery, deleteDelivery, updateEntregador };
